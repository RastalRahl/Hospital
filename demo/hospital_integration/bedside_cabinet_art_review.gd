...
const OUT := "res://repair_candidates/perspective_cabinet_v2/"
...
const FEET_Y := 330.0
const POSITIONS := [70.0, 180.0, 290.0, 390.0, 500.0, 640.0]
...
func _ready() -> void:
	...
	add_sprite(OUT + "bedside_cabinet_01_art_candidate_v2.png", POSITIONS[2])
	add_adult(POSITIONS[3])
	add_sprite("res://art/hospital_bed_standard_01__main.png", POSITIONS[4])
	add_sprite("res://art/waiting_chair_01__main.png", POSITIONS[5])
	queue_redraw()
	if "--bedside-cabinet-art-review" in OS.get_cmdline_user_args():
		await review()

func _draw() -> void:
	draw_rect(Rect2(0, 0, 720, 420), INK_2)
	draw_rect(Rect2(20, 80, 680, 48), WALL)
	draw_rect(Rect2(20, 128, 680, 232), FLOOR)
	draw_rect(Rect2(20, 126, 680, 2), TOP)
	for x in range(20, 701, 96):
	...
	draw_string(f, Vector2(24, 28), "BEDSIDE CABINET V2 — ART PILOT", HORIZONTAL_ALIGNMENT_LEFT, -1, 20, Color.WHITE)
	draw_string(f, Vector2(24, 58), "approved       V1 geometry       V2 art          adult          bed                 chair", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, Color("d8cabb"))
	for x in POSITIONS:
	...
	draw_string(f, Vector2(24, 394), "3x integer render • common bottom-center feet • unchanged scale anchors", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, Color("b9d8d3"))

func review() -> void:
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(OUT))
	await RenderingServer.frame_post_draw
	var image := get_viewport().get_texture().get_image().get_region(Rect2i(0, 0, 720, 420))
	assert(image.save_png(OUT + "bedside_cabinet_art_godot.png") == OK)
	...
	print("BEDSIDE_CABINET_ART_REVIEW_PASS: approved, V1 and V2 share feet; V2 rendered beside adult, bed and chair at integer scale")
	get_tree().quit()
