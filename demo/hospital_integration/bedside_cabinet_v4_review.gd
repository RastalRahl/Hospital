...
const OUT := "res://repair_candidates/perspective_cabinet_v4/"
...
const FEET_Y := 330.0
const POSITIONS := [70.0, 190.0, 310.0, 430.0, 580.0, 770.0]
...
func _ready() -> void:
	...
	add_sprite(OUT + "bedside_cabinet_01_v3_art.png", POSITIONS[1])
	add_sprite(OUT + "bedside_cabinet_01_art_candidate_v4.png", POSITIONS[2])
	...
	queue_redraw()
	if "--bedside-cabinet-v4-review" in OS.get_cmdline_user_args():
		await review()
...
func _draw() -> void:
	...
	draw_string(f, Vector2(24, 28), "BEDSIDE CABINET V4 — ART PILOT", HORIZONTAL_ALIGNMENT_LEFT, -1, 20, Color.WHITE)
	draw_string(f, Vector2(24, 58), "approved         V3 art          V4 art          adult             bed                     chair", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, Color("d8cabb"))
	for x in POSITIONS:
...
func review() -> void:
	...
	print("BEDSIDE_CABINET_V4_REVIEW_PASS: V4 rendered beside approved, V3, adult, bed and chair at integer scale")
	get_tree().quit()
