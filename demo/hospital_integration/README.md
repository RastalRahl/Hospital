...
This runs assertions, overwrites `overview.png`, and places disposable glass front/behind, open-door and debug captures in `.qa/`. Look for `HOSPITAL_SMOKE_PASS`; a zero exit alone is insufficient because the timeout can end a failed assertion run. Repository tests: `python -m pytest -q`.
