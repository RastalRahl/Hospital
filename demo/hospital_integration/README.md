...
**J** toggles the reference-only junction prototype and enables the refreshed wall family. Six explicit junction views replace seven side-wall sprites at the existing ground positions: northwest/northeast, the internal north branch, both south corners, and the divider's east-going full-height return. The north views extend 52 native pixels above the first side cell; the divider end spans its final two cells with a declared 16px visual extension. These are camera-aware corner views, not moved walls or new catalog entries. Three missing 8px-wide collision contacts are active only with J, drawn magenta with G. Existing collision rectangles, furniture/layout and `runtime_manifest.json` remain unchanged. Turning J off restores the original side sprites; V also exits junction mode. Startup remains approved walls with original junctions.

Use `launch.ps1 -Godot '<Godot executable>' -JunctionReview` for before/after captures, contact and figure-sort assertions (`WALL_JUNCTION_REVIEW_PASS`). `-JunctionSmoke` runs the existing full walkthrough/smoke with the prototype enabled (`HOSPITAL_SMOKE_PASS`). The six close-up pairs temporarily hide props so the joins can be seen; furnished overviews keep all props. Native views, explicit footprints/anchors, reproduction scripts and the review montage are in `repair_candidates/wall_junctions_v1/`. This prototype covers only the six current scene junctions, not arbitrary L/T/cross arrangements. Production promotion remains on hold for human review.
