...
- Junction prototype: [findings](../demo/hospital_integration/repair_candidates/wall_junctions_v1/findings.md) and [montage](../demo/hospital_integration/repair_candidates/wall_junctions_v1/junction_review_montage.png). Six deterministic native views connect the six existing solid-wall contacts; seven original side sprites are replaced only while J is enabled. The north returns connect the upper cap, a distinct branch preserves the continuous top band, and the lower divider/front corners now own the joined face/base. Three missing 8px collision contacts are added only in prototype mode. Codex inspected the furnished and unobstructed Godot views; **awaiting human visual review**, not production-approved.
...
- Latest completed junction verification: full suite **377 passed in 79.27s**; focused demo tests **9 passed**; **WALL_JUNCTION_REVIEW_PASS**; existing **HOSPITAL_SMOKE_PASS** with junctions enabled; **0 scene pixel changes outside six junction carriers and HUD**. All **1,040 protected files unchanged**, including the five user-owned files. Full-suite output and preservation result are in the junction folder's `test_results.txt` and `verification.json`. The initial full run found the bootstrap state document missing from the historical audit's allowed additions; an exact named allowance fixes this without exempting any baseline hash.

## Next priorities

1. Obtain the user's visual decision on the completed six-junction prototype, directly from the montage or J toggle in the stable demo. Production promotion remains on hold.
