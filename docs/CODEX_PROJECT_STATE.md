...
- [PERSPECTIVE_REPAIR_PLAN.md](PERSPECTIVE_REPAIR_PLAN.md) is the complete camera audit of the current inventory: **144 perspective repairs in nine batches + 80 perspective retains = 224 assets**. Retain means perspective-compatible, not otherwise release-approved. The repair program changes no logical counts.
- After architecture cleanup, approve the three-asset perspective calibration and complete R1–R9 before generating new roadmap assets. The next five new-content families remain **architecture 12; bathrooms/utility 18; signage/decor/safety 22; staff/admin 14; combined clinical completion 11**. Then exterior 17, food service 14, morgue 8, inventory/UI 13 and abandoned 20. This is planning, not production authorization.
...
- Perspective calibration V1: [findings](../demo/hospital_integration/repair_candidates/perspective_calibration_v1/findings.md), [Godot comparison](../demo/hospital_integration/repair_candidates/perspective_calibration_v1/perspective_calibration_godot.png). The user accepted its improved consistency but rejected its crude art quality. Preserve it only as a geometry/scale reference; do not propagate its styling.
- Bedside cabinet V2 is rejected as a release art baseline because its top, internal outlines and cupboard detail read too heavily. Bedside cabinet V3 is the active pilot: [findings](../demo/hospital_integration/repair_candidates/perspective_cabinet_v3/findings.md), [native comparison](../demo/hospital_integration/repair_candidates/perspective_cabinet_v3/bedside_cabinet_v3_review.png), [Godot comparison](../demo/hospital_integration/repair_candidates/perspective_cabinet_v3/bedside_cabinet_v3_godot.png). The 32x36 candidate uses 11 hard-alpha colors, one shallow top, exterior-weighted outlines and quiet large clusters at the same 1x1 bottom-center contract. Godot `BEDSIDE_CABINET_V3_REVIEW_PASS`; **awaiting human visual review**. Production/staging/manifest/approval remain unchanged.
...
4. Human-review the bedside cabinet V3 art pilot. If accepted, rebuild `reception_counter_straight_01` and `medical_cart_base_01` to the same exterior-outline and large-cluster bar while retaining the V1 perspective contract; review the three together before R1–R9. This includes brochure-rack/plant cleanup; the retained benches still receive their separate proportion/scale fix.
5. Once the 144-asset perspective program and retained scale exceptions are resolved, execute the next five new-content families in [ASSET_COVERAGE_STATUS.md](ASSET_COVERAGE_STATUS.md); maintain its concrete backlog against the manifest at every completed batch. Preserve the category budgets in [ASSET_ROADMAP.md](ASSET_ROADMAP.md).

Use cost-conscious iteration: existing scripts/contracts, targeted checks, full suite once per completed batch, Godot as primary practical test. No bespoke validator framework for ordinary assets or bulk generation without instruction. Repair current perspective before adding to the revision-2 target of 373 meaningful assets; see the canonical roadmap, coverage status and perspective repair plan.
