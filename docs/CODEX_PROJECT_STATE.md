...
1. Current review: `demo/hospital_integration/solid_wall_review.tscn` (F6), with evidence in `repair_candidates/solid_wall_review_v1/`. The user confirmed removing the unnecessary central glass enclosure. All 22 glass renderers and their collision/debug anchors are excluded; examination furniture is retained. Glass experiments are parked. No replacement windows are planned unless requested. Next resolve the solid foreground wall's visibility problem and review the complete room. Production promotion remains on hold.
