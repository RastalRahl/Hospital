...
1. Resolve foreground visibility after the actual-hospital review: `demo/hospital_integration/repair_candidates/hospital_foundation_integration_v1/candidate_godot.png`, via `hospital_foundation_review.tscn` (F6, H compares). The coherent full-height Foundation candidate now uses the existing hospital wall contacts plus four declared corner/return contacts; furniture, floor, glass, doors and their positions remain unchanged. The existing actual-hospital smoke passes with the candidate. Codex inspected connected north/divider/door joins, but the full front hides nearly the whole figure when close behind it (`front_near_godot.png`). Keep the construction/finish; solve this visibility/camera choice before production promotion. Normal startup and the production low-front contract remain unchanged.
