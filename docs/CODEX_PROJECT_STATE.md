...
1. Review the refined furnished construction test: `demo/hospital_integration/repair_candidates/wall_logic_reference_v2/front_godot.png`, runnable via `wall_logic_refined.tscn` (F6). Front-room side/divider returns now share the low front's 12px cut plane, removing isolated tall terminal faces. Full-height back/internal walls, shared L/T/cross logic, shaded jambs and stronger cap/floor contrast remain. Existing bed/cabinet and a movable 26x46 figure test scale, doorway clearance and depth ordering. Codex prefers this to reference V1; Foundation finishing and application to the full hospital still require review/integration. Both reference V1 and the earlier wall_junctions_v2 patch candidate remain preserved. Production promotion stays on hold.
