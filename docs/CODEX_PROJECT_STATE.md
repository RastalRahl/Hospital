...
1. Review the coherent single-reference baseline: `demo/hospital_integration/repair_candidates/single_reference_room_v1/room_godot.png`, via `reference_room.tscn` (F6). The user authorized a complete reference convention after Codex acknowledged that mixed-height strips were its own workaround. SmallBurg's supplied `examples/cosy_house.png` is the sole construction reference. All four walls use 44px height, connected narrow caps and full horizontal faces, including the front. Existing furniture/26x46 figure demonstrate scale and expected front-wall occlusion. This is an explicit study alternative to the production low-front policy, not its replacement. Internal T/cross layouts and Foundation finishing remain unproven; older candidate evidence is preserved. Production promotion remains on hold.
