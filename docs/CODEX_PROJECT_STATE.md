...
1. Review the actual-hospital glass refresh: `demo/hospital_integration/repair_candidates/glass_enclosure_refresh_v1/candidate_godot.png`, via `hospital_glass_review.tscn` (F6; O compares glass, H walls). The user flagged the central glass as wall-like. This candidate reduces glazing alpha to 56/255, removes repeated heavy bars and cleans the back-corner inner rail starts. All 22 placements retain canvas/support masks, anchors, sorting, openings and collision; their four logical parents remain pending. Codex inspected furnished/unobstructed/behind views and prefers the lighter enclosure. New frame/alpha regions need explicit contract revision before promotion; placement-specific review textures are not a production view package. After glass review, resolve the solid foreground wall's visibility problem and review the complete room. Production promotion remains on hold.
