...
1. Review the corrected lower side joins: `demo/hospital_integration/repair_candidates/wall_logic_reference_v3/front_godot.png`, via `wall_logic_refined.tscn` (F6). User rejected V2's side caps starting midway down the full wall face. V3 follows the reference wall-top construction: three narrow cutaway edges now join the upper border to the lower return, removing shared internal outlines. This is a camera-aware reveal, not a change to physical height or footprints. Only 756 pixels per native view change, inside the three joins; doors, furniture, collision and figure sorting are unchanged. Foundation finishing and full hospital integration remain outstanding. Earlier reference and patch candidates remain preserved; production promotion stays on hold.
