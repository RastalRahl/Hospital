...
1. Review the connected single-convention proof: `demo/hospital_integration/repair_candidates/connected_reference_rooms_v1/corridor_crossing_godot.png`, via `connected_reference_rooms.tscn` (F6). Two furnished rooms connect through a deeper corridor, with L/T/cross caps, two room doorways and a front exit. All walls remain 44px; mixed-height patches are inactive. The figure walkthrough reaches both rooms and the exit. Codex inspected the continuous joins and visible corridor; the cross's southern arm has a plain exposed terminal face. Full-front occlusion remains the acknowledged tradeoff. After human visual acceptance, apply Foundation finishes and integrate the hospital; do not promote production assets yet. Earlier single-room and patch evidence remains preserved.
