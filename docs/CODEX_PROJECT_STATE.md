...
1. Review the completed reference-led empty construction test: `demo/hospital_integration/repair_candidates/wall_logic_reference_v1/godot_review.png`, runnable via `wall_logic_test.tscn` (F6) in the existing demo. Shared ground-column construction connects L/T/cross caps without special patches. A uses full height; B explicitly shows the tall terminal faces caused by lowering the front strip. Codex recommends the shared construction logic; the mixed-height terminal appearance needs human review before hospital integration. V2 remains a preserved baseline, not a recommended production candidate. Production promotion stays on hold.
